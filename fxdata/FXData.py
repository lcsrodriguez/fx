...
class FXData:
    ...
    __slots__: dict = ("cwd", "keepCSV", "keepGZIP", "castDatatime", "outputCSV", "outputPQT")
    ...
    def _handleIntemediaryFiles(self) -> None:
        ...
        if not self.keepGZIP:
            fs = glob.glob(f"{self.cwd}/{TMP_FOLDER}/*.gz")
            for f in fs:
                os.remove(f)
    ...
    def getData(self,
        ...
        if pair not in AVAILABLE_SYMBOLS[dataType]:
            raise Exception(f"Pair {pair} not available for requested type of data ({dataType})")
        ...
        for y in rY:
            for w in rW:
                ...
                if len(THREADS_POOL) <= THREADS_POOL_LIMIT:
                    print(f"Processing week {y}-{w}", end=" ")
                    t = threading.Thread(target=self.getTickData if dataType == DataType.TICK else self.getCandleData,
                                         kwargs={
                                             "pair": pair,
                                             "yr": y,
                                             "wk": w,
        ...
        if self.outputCSV:
            #print("writing csv")
            fdf.to_csv(f"{self.cwd}/{OUT_FOLDER}/csv/{output_filename}.csv")
        if self.outputPQT:
            import pyarrow as pa
            import pyarrow.parquet as pq
            #print("Writing pqt")
            table = pa.Table.from_pandas(df=fdf)
            pq.write_table(table=table, where=f"{self.cwd}/{OUT_FOLDER}/parquet/{output_filename}.parquet")
        return fdf
    ...
    def _getDataFrame(self, config: Config, p: bool = True) -> Union[pd.DataFrame, None]:
        ...
        with open(f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv.gz", 'wb') as f:
        ...
        if p:
            ...
            with gzip.open(f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv.gz", 'rb') as f_in:
                with open(f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv", 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)

            # Reading unprocessed/binary CSV file
            fi = open(f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv", 'rb')
            data = fi.read()
            fi.close()

            # Processing + Writing CSV file
            fo = open(f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv", 'wb')
            fo.write(data.replace(b'\x00', b''))
            fo.close()

            # Reading CSV file
            df = pd.read_csv(filepath_or_buffer=f"{self.cwd}/{TMP_FOLDER}/{config.getFilename()}.csv",
                             sep=",",
                             on_bad_lines='skip')
            if self.castDatatime:
